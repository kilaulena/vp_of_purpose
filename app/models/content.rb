...
class Content < ActiveRecord::Base
  ...
  def rss_description(xml)
    if respond_to?(:user) && self.user && self.user.name
      rss_desc = "<hr /><p><small>#{_('Original article written by')} #{self.user.name} #{_('and published on')} <a href='#{blog.base_url}'>#{blog.blog_name}</a> | <a href='#{self.permalink_url}'>#{_('direct link to this article')}</a></small></p>"
    else
      rss_desc = ""
    end
